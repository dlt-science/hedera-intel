...
# Examples of fetch: 
# python3 scripts/fetch/fetch-account-txs.py 0.0.70 --max_limit 200
# with filter: python3 scripts/fetch/fetch-account-txs.py 0.0.70 --max_limit 200 --filter_account 0.0.10
...
df.to_csv(os.path.join(RESULTS_PATH, csv_file), index=False)
...
if args.match_string:
    ...
    with open(os.path.join(RESULTS_PATH, csv_file), 'r') as csv_in, open(os.path.join(RESULTS_PATH, output_file), 'w', newline='') as csv_out:
